...
from backend_server.models.map import twos_comp_byte
...
def test_listener_callback(create_subscription_mock,
                           mock_convertDataToBase64Str):  # ASK MAJEED Permission to precise types
...
@patch("rclpy.node.Node.create_subscription")
@patch("backend_server.ros.managers.map.twos_comp_byte")
...
@patch("rclpy.node.Node.create_subscription")
@patch("backend_server.models.map.twos_comp_byte")
...
def test_create_data_array(append_grid_data_to_array_mock, twos_comp_byte_mock, create_subscription_mock):
    ...
    expected_data = array('b',
                          [0x42, 0x4D, 127, 127, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x36, 0x00, 0x00, 0x00, 0x28, 0x00,
                           0x00, 0x00, 10, 0, 0x00, 0x00, 20, 0, 0x00, 0x00, 0x01, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00,
                           0x00, 127, 127, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
@patch("rclpy.node.Node.create_subscription")
@patch("backend_server.ros.subscribers.map.append_point_value_to_data")
@patch("backend_server.ros.subscribers.map.add_padding_to_data")
...
def test_append_point_value_to_data(twos_comp_byte_mock, create_subscription_mock):
    create_subscription_mock.return_value = MagicMock()
    map_subscriber = MapSubscriber()
    twos_comp_byte_mock.side_effect = lambda \
            x: 127 if x > 127 else x  # Mock twos_comp_byte to return a valid signed char value
...
@pytest.mark.asyncio
@patch("backend_server.websocket.emitter.sio.emit")
