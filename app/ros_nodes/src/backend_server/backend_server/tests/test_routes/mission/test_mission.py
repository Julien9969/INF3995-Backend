...
@pytest.mark.asyncio
@patch("backend_server.websocket.base.sio.emit")
@patch("backend_server.mission.StatusUpdate", autospec=True)
async def test_get_mission_status(mock_StatusUpdate, mock_sio_emit):
    ...
    mission = Mission()
    await mission.get_mission_status(1)
    mock_StatusUpdate.assert_called_once()
    mock_sio_emit.assert_called_once_with(WebsocketMessage.MISSION_STATUS.value, "empty", to=1)


@pytest.mark.asyncio
@patch("backend_server.models.logs", autospec=True)
@patch("backend_server.websocket.base.mission.sio.emit", autospec=True)
...
async def test_set_mission_start(mock_mission, mock_sio_emit, mock_logs):
    ...
    mission = Mission()
    await mission.set_mission_start(1)
    mock_mission.assert_called_once()
    mock_sio_emit.assert_called_once_with(WebsocketMessage.MISSION_START.value, "empty", to=1)


@pytest.mark.asyncio
async def test_start_record_map():
    with patch("backend_server.websocket.base.sio.emit", autospec=True) as mock_sio_emit, \
            patch("backend_server.ros.managers.map.MapManager", autospec=True) as mock_MapManager, \
            patch("backend_server.websocket.emitter.send_log", autospec=True) as mock_send_log:
        mock_sio_emit.side_effect = side_effect
        mock_send_log.return_value = AsyncMock()
        mock_MapManager.start_map_listener.return_value = AsyncMock()
        mission = Mission()
        await mission.start_record_map(1)
        mock_send_log.assert_called_once()
        mock_sio_emit.assert_called_once_with(WebsocketMessage.MISSION_MAP.value, "empty", to=1)


@pytest.mark.asyncio
@patch("backend_server.websocket.event_handlers.mission.sio.emit")
@patch("backend_server.models.mission", autospec=True)
async def test_get_mission_status(mock_StatusUpdate, mock_sio_emit):
    mock_sio_emit.side_effect = side_effect
    mock_status = MagicMock()
    mock_status.to_json.return_value = "empty"
